...
def root(rows):
	all_parents = set()
	all_children = set()
	for name, weight, children in rows:
		all_parents.add(name)
		all_children = all_children | set(children)

	return next(iter(all_parents - all_children))

def soln2(rows):
	Node = namedtuple("Node", ["name", "weight", "children"])
	tree = {name: Node(name, weight, children) for name, weight, children in rows}

	def actual_weight(node):
		total = tree[node.name].weight
		for child in tree[node.name].children:
...
def main():
	tests()
	rows = parse(HARD)
	print(f"Solution 1: {root(rows)}")
	print(f"Solution 2: {soln2(rows)}")

def tests():
	assert parse_row(TINY) == ("fwft", 72, ["ktlj", "cntj", "xhth"]), parse_row(TINY)
...
def parse_row(row):
	...
	def parse_name(row):
		return RE_NAME.findall(row)[0]

	def parse_children(row):
		if "->" in row:
