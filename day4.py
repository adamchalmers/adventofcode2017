...
def sorted_words(line):
	return [str(sorted(l)) for l in line]

def main():
	passes = [w.split() for w in input_text.split("\n")]
	valid = [line for line in passes if all_unique(line)]
	print(len(valid))
	secureValid = [line for line in passes if all_unique(sorted_words(line))]
	print(len(secureValid))

input_text = """nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
