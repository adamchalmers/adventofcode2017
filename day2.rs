fn main() {
    println!("Hello, world!");
    let total: i32 = hard_input().iter().map(checksum).sum();
    let total2: i32 = hard_input().iter().map(|x| checksum2(x).unwrap()).sum();
...
fn hard_input() -> Vec<Vec<i32>> {
	let grid = String::from("278	1689	250	1512	1792	1974	175	1639	235	1635	1690	1947	810	224	928	859
...
114	605	94	136	96	167	553	395	164	159	284	104	530	551	544	18");
	let to_row = |r: &str| r.split("\t").map(|x| x.parse::<i32>().unwrap()).collect();
	grid.split("\n").map(to_row).collect()
}