...
def parse(row):
	(register, operation, operand, _, lhs, relation, rhs) = row.split(" ")
	return (register, operation, int(operand), lhs, relation, int(rhs))
...
def solve1(rows):
	...
	historical_max = 0
	for (register, operation, operand, lhs, relation, rhs) in rows:
		if RELATIONS[relation](registers[lhs], rhs):
			registers[register] += OPERATIONS[operation] * operand
			if registers[register] > historical_max:
				historical_max = registers[register]
	return max(registers.values()), historical_max


def main():
	rows = [parse(s.strip()) for s in HARD.split("\n")]
	final_max, historical_max = solve1(rows)
	print("Soln 1: %d" % final_max)
	print("Soln 2: %d" % historical_max)
